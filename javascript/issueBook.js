...
async function displaying() {
    let list = "";
    let res = await fetch("http://localhost:8080/Library_With_MVC/getAllRequest");
    ...
    if (true) {
        for (let r of requests) {
            if (uid == r.uid && r.bookId == bookId) {
                ...
                let url = "http://localhost:8080/Library_With_MVC/saveIssueBook";
                let par = {
                    method: 'POST',
                    headers: {
...
async function requestDelete(rid) {
    console.log("delete")
    await fetch("http://localhost:8080/Library_With_MVC/deleteRequest?id=" + rid, {
        method: "DELETE"
