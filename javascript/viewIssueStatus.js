...
async function displaying() {
  let resp = await fetch("http://localhost:8080/Library_With_MVC/getAllIssueBook");
  ...
  let resp1 = await fetch("http://localhost:8080/Library_With_MVC/getAllRequest");
...
async function deleteIssueBook(issueId,bookId,uid){ 
  await fetch("http://localhost:8080/Library_With_MVC/deleteIssueBook?id=" + issueId, {
    method: "DELETE"
})
...
async function userDetailsUpdate(uid, bookId) {
  ...
  let res1 = await fetch("http://localhost:8080/Library_With_MVC/getById?id="+uid);
  let user = await res1.json();
  let res2 = await fetch("http://localhost:8080/Library_With_MVC/getBookById?id="+bookId);
  ...
  if (uid == user.uid) {
      ...
      console.log(user.no_of_books_taken + "user");
      let url2 = 'http://localhost:8080/Library_With_MVC/update';
      let par2 = {
          method: 'POST',
          headers: {
  ...
  if (bookId == book.bookId) {
      ...
      console.log(book.no_of_books + "books");
      let url = "http://localhost:8080/Library_With_MVC/updateBook";
      let par = {
          method: 'POST',
          headers: {
              'Accept': 'application/json',
